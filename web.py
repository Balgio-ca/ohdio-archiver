"""Interface web de l'archiveur OHdio + planificateur d'archivage automatique.
...
def every_hours() -> float:
    return float(ohdio.config.get("every_hours", 24))


def archive_statuses() -> dict[int, str]:
    out = {}
    root = ohdio.archive_dir() / "Livres audio"
    if root.exists():
        for m in root.rglob(ohdio.MANIFEST):
            try:
...
def run_job(ids: list[int] | None = None) -> None:
    ...
    with lock:
        ...
        try:
            shows = ohdio.load_catalog(refresh=not ids)
            if ids:
                wanted = set(ids)
                shows = [s for s in shows if s["globalId"] in wanted]
            else:
                ab = ohdio.config.get("audiobooks", {})
                shows = [s for s in shows if ohdio.selected(s, ab.get("include", []), ab.get("exclude", []))]
            state["last_summary"] = ohdio.sync(shows)
        except Exception as e:
            ohdio.log(f"ERREUR : {e}")
        finally:
...
def scheduler() -> None:
    ...
    while True:
        ...
        while not pending_ids:
            ...
            if logged != nxt:
                ohdio.log(f"Prochain passage automatique : {state['next_run'].replace('T', ' ')}")
                logged = nxt
            if time.time() >= nxt:
                break
            if wake.wait(5):
...
def books_payload() -> dict:
    shows = ohdio.load_catalog(refresh=False)
    ab = ohdio.config.get("audiobooks", {})
    statuses = archive_statuses()
    books = []
    for s in shows:
        cat = ohdio.category_of(s)
        books.append({
            "id": s["globalId"], "title": s["title"], "author": s.get("subtitle") or "",
            "category": cat, "jeunesse": bool(ohdio.AGE_RE.match(cat)),
            "picture": (s.get("picture") or "").replace("{width}", "160").replace("{ratio}", "1x1"),
            "selected": ohdio.selected(s, ab.get("include", []), ab.get("exclude", [])),
    ...
    for b in books:
        c = cats.setdefault(b["category"], {
            ...
            "enabled": (not inc or ohdio.matches(b["category"], inc)) and not ohdio.matches(b["category"], exc)})
        c["count"] += 1
...
def status_payload() -> dict:
    return {**state, "progress": dict(ohdio.PROGRESS), "log": list(ohdio.LOG)[-150:],
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        ab = ohdio.config.setdefault("audiobooks", {})
        ...
        if path == "/api/settings":
            ...
            ohdio.config["every_hours"] = int(hours) if hours.is_integer() else hours
            ohdio.save_config()
        ...
        if path == "/api/refresh":
            ...
            ohdio.load_catalog(refresh=True)
            return self.json({"ok": True})
...
def serve(port: int) -> None:
    ohdio.log(f"Interface web sur le port {port}")
