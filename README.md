...
- **Aucune affiliation.** Ce projet n'est ni affilié, ni approuvé, ni soutenu par la Société Radio-Canada / CBC ou par yodio.ca. Les noms et marques cités appartiennent à leurs propriétaires respectifs.
...
Pour soutenir les créateurs, écoutez aussi les œuvres sur [Radio-Canada](https://ici.radio-canada.ca/) et achetez les livres qui vous plaisent.
...
1. Le catalogue vient de l'API publique de [yodio.ca](https://yodio.ca/shows/audiobooks), un client alternatif de la plateforme audio de Radio-Canada.
...
```bash
git clone https://github.com/Balgio-ca/yottio-archiver.git
cd yottio-archiver
cp .env.example .env      # adapter PUID, PGID et YOTTIO_ARCHIVE_PATH
...
Dans Audiobookshelf, créez une bibliothèque qui pointe vers `<YOTTIO_ARCHIVE_PATH>/Livres audio`. Chaque dossier contient un `metadata.json` lu par Audiobookshelf, qui fournit les vrais auteurs, le genre, l'éditeur et l'année.
...
```bash
python3 yottio.py catalog                     # décompte par catégorie
python3 yottio.py list                        # livres retenus par les filtres
python3 yottio.py download --dry-run          # simulation
python3 yottio.py download                    # archivage (relancer pour reprendre)
python3 yottio.py download --include roman essai --limit 5
python3 yottio.py download --ids 105711       # un livre précis
python3 yottio.py verify --deep               # SHA-256 + relecture de toute l'archive
python3 yottio.py serve                       # interface web + archivage automatique
```

Dans Docker, les mêmes commandes passent par `docker compose run --rm yottio <commande>`.
